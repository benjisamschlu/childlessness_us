...
##  and race/ethnicity with a Janoshek curve using CPS and NSFG data. 
##
## 
        ...
        df.i |> 
                left_join(
                        df.stan |> 
                                ...
                                dplyr::select(
                                        edu_i, race_i, cohort_i, age_i, obs_i
                                ),
                        by = c("edu_i", "race_i", "cohort_i", "age_i")
                ) |> 
                filter(
                        !is.na(obs_i)
                ) |> 
...
edu_g <- rep(unique(df.stan$edu_i), each = 3)

# Index of race for each group
race_g <- rep(unique(df.stan$race_i), times = length(unique(df.stan$edu_i)))
...
df.fit <- 
        ...
        mutate(
                ...
                race_eth = df.i.race.edu$race[g],
                edu = df.i.race.edu$edu[g]
        ) |> 
        full_join(
                df.stan,
                by = c("cohort5", "race_eth", "edu", "age")
        )
# Save df
saveRDS(
        df.fit,
